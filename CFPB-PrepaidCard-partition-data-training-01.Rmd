...
title: "CFPB: Consumer Dispute?  classification:: CFPB-PrepaidCard-partition-data-training-01.html CFPB-PrepaidCard-partition-data-training-01.md CFPB-PrepaidCard-partition-data-training-01"
author: "bdanalytics"
...
glbOut <- list(pfx = "CFPB-PrepaidCard-partition-data-training-01.html CFPB-PrepaidCard-partition-data-training-01.md CFPB-PrepaidCard-partition-data-training-01_")
...
glbChunks[["last"]] <- "partition.data.training" #NULL #default: script will save envir at end of this chunk
glbChunks[["inpFilePathName"]] <- NULL # "data/CFPB-PrepaidCard-partition-data-training-01_partition.data.training.RData" #mysavChunk(glbOut$pfx, glbChunks[["last"]]) # called from myevlChunk
            ...
            warning("No features in selected model are statistically important") else {
              ...
              for (pltIx in seq(1, 2)) {
                if ((nrow(featsimp_df) >= pltIx) && 
                    !is.na(featsimp_df[pltIx, "feat.interact"])) {
                  pltFts[[pltIx]] <- c(feat_x = featsimp_df[pltIx, "feat"], 
                                       feat_y = featsimp_df[pltIx, "feat.interact"]) 
                } else
                if ((nrow(featsimp_df) == pltIx) && 
                    is.na(featsimp_df[pltIx, "feat.interact"])) {
                  pltFts[[pltIx]] <- c(feat_x = ifelse(pltIx == 1, 
                                                       ".rownames", 
                                                       featsimp_df[pltIx + 1, "feat"]), 
                                       feat_y = featsimp_df[pltIx, "feat"])
                }
              }
