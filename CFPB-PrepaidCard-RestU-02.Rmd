...
title: "CFPB: Consumer Dispute?  classification:: CFPB-PrepaidCard-RestU-02"
author: "bdanalytics"
...
setwd("~/Documents/Work/DataScience/CFPB")
...
- Feature Engineering:
  - DateRecieved, DateSenttocompany: 
    - Add Federal, State & City holidays for each year
...
#stop(here"); glb2Sav(); glbObsAll <- savObsAll
    ...
    if (grepl("^%<d-%", glbFeatsCluster)) {
        ftr <- str_trim(unlist(strsplit(glbFeatsCluster, "%<d-%"))[2])
        glbFeatsCluster <- eval(parse(text = ftr))
    }        
...
#stop(here"); glb2Sav()    
    ...
    for (grp in grp_ids) {
        ...
        dstns_mtrx[dstns_mtrx < 0] <- 0 # Why, oh Why ?
...
if (all(is.na(glbObsNew[, glb_rsp_var]))) {
    ...
    lclcastMtx <- function(obsDf) {
        ...
        #   Find features (ignore glbFeatsExclude; why ???
        #       incl. glbFeatsExclude increases run-time unnecessarily ?
        ...
        ftr <- setdiff(names(obsDf), 
                        c(glbFeatsId, glb_rsp_var_raw, glb_rsp_var, 
                          glbFeatsExclude,
                          # Exclude internally computed features
                          grep("^\\.", names(obsDf), value = TRUE)))
        ftrChr <- myfind_fctr_cols_df(obsDf[, ftr])
        ftrNonChr <- setdiff(ftr, ftrChr)
        if (length(ftrNonChr) == 0) {
          #stop("lclCorMtx: this will not work with empty ftrNonChr")
          ftrNonChr = ".pos"
        }  
...
#             as.character(rspvar_freq_df[, glb_rsp_var_raw])
        ...
        TrnNewSmy <- mygetMatrixSimilarity(x = mtxObsTrn, y = mtxObsNew, weights = ftrWgt)
        ...
        TrnNewSmy <- merge(glbObsTrn[, c(glbFeatsId, glbFeatsCategory, glb_rsp_var)],
                           TrnNewSmy, by = "row.names", all.x = TRUE)
        if (rspGrpVar != glb_rsp_var_raw) 
        ...
        if (sum(thspik, na.rm = TRUE) != length(thspik)) {
          # initialize NA's to min(thspik) / 100.0
          thspik[is.na(thspik)] <- min(thspik, na.rm = TRUE) / 100.0
          # scale to sum up to 1
          thspik <- thspik / sum(thspik)
          assertthat::are_equal(sum(thspik), 1.0)
        }
    ...
    OOBFitSmy <- merge(glbObsOOB[, c(glbFeatsId, glbFeatsCategory, glb_rsp_var)],
                       OOBFitSmy, by = "row.names", all.x = TRUE)

    OOBNewSmy <- mygetMatrixSimilarity(mtxObsOOB, mtxObsNew, weights = ftrWgt)
    OOBNewSmy <- merge(glbObsOOB[, c(glbFeatsId, glbFeatsCategory, glb_rsp_var)],
                       OOBNewSmy, by = "row.names", all.x = TRUE)

    FitNewSmy <- mygetMatrixSimilarity(mtxObsFit, mtxObsNew, weights = ftrWgt)
    FitNewSmy <- merge(glbObsFit[, c(glbFeatsId, glbFeatsCategory, glb_rsp_var)],
                       FitNewSmy, by = "row.names", all.x = TRUE)
    
    metricsDst <- setdiff(names(OOBFitSmy), 
                          c("Row.names", glbFeatsId, glbFeatsCategory, glb_rsp_var))
    smyDf <-  as.data.frame(rbind(colMeans(OOBFitSmy[, metricsDst], na.rm = TRUE),
                                  colMeans(OOBNewSmy[, metricsDst], na.rm = TRUE),
                                  colMeans(FitNewSmy[, metricsDst], na.rm = TRUE)))
...
# This fn is repeated again in select.features (temporarily - clean it later)
lclComputeLvlCategory <- function() {
      if (glb_is_classification)
    ...
    return(glbLvlCategory)
}
if (!is.null(glbFeatsCategory)) 
  glbLvlCategory <- lclComputeLvlCategory()
...
if (!is.null(glbChunks$first) && (glbChunks$first == "select.features") &&
    (is.null(knitr::opts_current$get(name = 'label')))) # not knitting
    myloadChunk(glbChunks$inpFilePathName, keepSpec = c("glbMdlFamilies", "glbFeatsExclude", "glbMdlSltId"), dropSpec = c(NULL))
```

## Step ``r mydsp_chunk(glb_chunks_df)``
```{r select.features, cache=FALSE, echo=FALSE, eval=myevlChunk(glbChunks, glbOut$pfx, keepSpec = c("glbMdlFamilies", "glbFeatsExclude", "glbMdlSltId"), dropSpec = c(NULL))}
...
numeric_indepVar <- numeric_indepVar[!grepl(".POSIX", numeric_indepVar, fixed = TRUE)]
...
uY <- factor(as.numeric(glbObsFit[, glb_rsp_var]) - 1)
uXFtr <- setdiff(myextract_actual_feats(mygetIndepVar(glb_feats_df)),
                 myfind_fctr_cols_df(glbObsFit))
uXFtr <- uXFtr[!grepl(".POSIX", uXFtr, fixed = TRUE)]
uXFtr <- union(uXFtr, glbFeatsId)
balObsFit <- ubBalance(X = glbObsFit[, uXFtr], Y = uY, 
                       # type = "ubOSS", verbose=TRUE)
                       type = "ubSMOTE", verbose=TRUE)
#balObsFit <- subset(glbObsFit, ComplaintID %in% balObsFit$X$ComplaintID)
balObsFit <- subset(glbObsFit, ComplaintID %in% balObsFit$X$ComplaintID)
glbObsFit <- balObsFit
glbObsOOB <- glbObsTrn[!(glbObsTrn$ComplaintID %in% glbObsFit$ComplaintID), ]
glbObsAll[glbObsAll[, glbFeatsId] %in% 
              glbObsFit[, glbFeatsId], ".lcn"] <- "Fit"
glbObsTrn[glbObsTrn[, glbFeatsId] %in% 
              glbObsFit[, glbFeatsId], ".lcn"] <- "Fit"
...
glbObsTrn[glbObsTrn[, glbFeatsId] %in% 
              glbObsOOB[, glbFeatsId], ".lcn"] <- "OOB"
# This fn is defined earlier in partition.data.training (temporarily - clean it later)
dsp_class_dstrb <- function(obs_df, location_var, partition_var) {
    xtab_df <- mycreate_xtab_df(obs_df, c(location_var, partition_var))
    rownames(xtab_df) <- xtab_df[, location_var]
    xtab_df <- xtab_df[, -grepl(location_var, names(xtab_df))]
...
mltd_models_df[, "method"] <- sapply(mltd_models_df$id, function (id)
  { thsId <- unlist(strsplit(id, "#")); return(thsId[length(thsId)]) })
...
#           theme(axis.text.x = element_text(angle = 45,hjust = 1)))
print(gp <- ggplot(data = mltd_models_df, aes(x = id)) + 
                     geom_bar(aes(y = value, color = method, fill = method), 
                              stat = "identity") + 
        geom_errorbar(data=mrgdCI_models_df, 
            mapping=aes(x= id, ymax=value.Upper, ymin=value.Lower), width=0.5) + 
          facet_grid(label ~ data, scales="free") + 
...
print(ggplot(caret::varImp(glb_models_lst[[glbMdlSltId]]), top = 10))
...
glb_analytics_diag_plots <- function(obs_df, mdl_id, prob_threshold=NULL, title = NULL) {
    ...
    #         stop("not implemented yet")
        ...
        for (var in featsimp_df$feat) {
            ...
            pltDf <- obs_df %>%
              tidyr::gather_(key_col = "var", value_col = "value", 
                             gather_cols = c(glb_rsp_var, rsp_var_out)) %>%
              dplyr::filter(!is.na(value))
            print(ggplot(data = pltDf, mapping = aes(x = var, y = value)) + 
                    geom_point(aes(color = value)) + 
                    geom_jitter(aes(color = value)) + 
                    facet_wrap("var"))
            # print(myplot_scatter(pltDf, var, "value", colorcol_name = "var",
            #                     facet_colcol_name = "var", jitter = TRUE) + 
            #               guides(color = FALSE))
        }
    }
    ...
    if (glb_is_classification) {
        ...
        if (is.null(featsimp_df) || (nrow(featsimp_df) == 0))
            warning("No features in selected model are statistically important") else {
              pltFts <- list()
              for (pltIx in seq(1, 2)) {
                if (nrow(featsimp_df) <= pltIx)
                 stop("glb_analytics_diag_plots: sitn needs refactoring/recoding") else {
                  if (!is.na(featsimp_df[pltIx, "feat.interact"])) {
                    pltFts[[pltIx]] <- c(feat_x = featsimp_df[pltIx, "feat"], 
                                         feat_y = featsimp_df[pltIx, "feat.interact"]) 
                  } else {
                    pltFts[[pltIx]] <- c(feat_x = featsimp_df[pltIx + 0, "feat"], 
                                         feat_y = featsimp_df[pltIx + 1, "feat"])
                  }
                }
              }
              for (pltIx in seq(1, length(pltFts))) {
                gp <- myplot_prediction_classification(df = obs_df, 
                                                       feat_x = pltFts[[pltIx]]["feat_x"],
                                                       feat_y = pltFts[[pltIx]]["feat_y"],
                                                       rsp_var = glb_rsp_var, 
                                    ...
                                    title = ifelse(is.null(title), 
                                                   sprintf("Prediction accuracy of %s", mdl_id), 
                                                   title))
                gpfl <- gp
                print(gp)
                png(paste0(glbOut$pfx, 
                           "Predictions_", mdl_id, "_", 
                           pltFts[[pltIx]]["feat_x"], "_", pltFts[[pltIx]]["feat_y"], ".png"), 
                    width = 480 * 2, height = 480 * 4)
                print(gpfl)
                dev.off()
              }  
            }  
...
if (!all(is.na(glbObsNew[, glb_rsp_var]))) {    
    ...
    # unbalanced adjustment
    rm(balObsFit)
    uY <- factor(as.numeric(glbObsTrn[, glb_rsp_var]) - 1)
    uXFtr <- setdiff(myextract_actual_feats(mygetIndepVar(glb_feats_df)),
                     myfind_fctr_cols_df(glbObsFit))
    uXFtr <- uXFtr[!grepl(".POSIX", uXFtr, fixed = TRUE)]
    uXFtr <- union(uXFtr, glbFeatsId)
    balObsTrn <- ubBalance(X = glbObsTrn[, uXFtr], Y = uY, 
                           # type = "ubOSS", verbose=TRUE)
                           type = "ubSMOTE", verbose=TRUE)
    #balObsFit <- subset(glbObsFit, ComplaintID %in% balObsFit$X$ComplaintID)
    balObsTrn <- subset(glbObsTrn, ComplaintID %in% balObsTrn$X$ComplaintID)
    glbObsTrn <- balObsTrn
    #glbObsOOB <- glbObsTrn[!(glbObsTrn$ComplaintID %in% glbObsFit$ComplaintID), ]
    # glbObsAll[glbObsAll[, glbFeatsId] %in% 
    #               glbObsFit[, glbFeatsId], ".lcn"] <- "Fit"
...
glb_analytics_diag_plots(obs_df = glbObsNew, mdl_id = glbMdlFnlNslId, 
...
for (clmn in names(glbObsOut$vars))
    if (!grepl("^%<d-%", glbObsOut$vars[[clmn]]))
        obsOutSelDf[, clmn] <- obs_df[, glbObsOut$vars[[clmn]]] else {
            feat <- str_trim(unlist(strsplit(glbObsOut$vars[[clmn]], "%<d-%"))[2])
            feat <- gsub("glbMdlFnlNslId", "glbMdlSltId", feat)
            obsOutSelDf[, clmn] <- eval(parse(text = feat))
    }        

if (glb_is_classification) {
    rsp_var_out <- paste0(mygetPredictIds(glb_rsp_var)$value, glbMdlFnlNslId)
    ...
    predctId <- mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value
    NewLvlCategory <- 
...
#     sav_ranges_new_df <- ranges_new_df; all.equal(sav_ranges_new_df, ranges_new_df)    
    ...
    rsp_var_out_OOB <- mygetPredictIds(glb_rsp_var, glbMdlSltId)$value
    rsp_var_out_new <- mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value
...
if (glb_is_classification && glb_is_binomial)
    print(glb_models_df[glb_models_df$id == glbMdlSltId, 
                        "opt.prob.threshold.OOB"])
print(sprintf("glbMdlSltId: %s", glbMdlSltId))
print(sprintf("glbMdlFnlNslId: %s", glbMdlFnlNslId))
print(subset(glbgetDisplayModelsDf(), select = -id))

if (glb_is_regression) {
    print(sprintf("%s OOB RMSE: %0.4f", glbMdlSltId,
            glb_models_df[glb_models_df$id == glbMdlSltId, "min.RMSE.OOB"]))
}    

if (glb_is_classification) {
    print(sprintf("%s OOB confusion matrix & accuracy: ", glbMdlSltId))
    print(t(confusionMatrix(glbObsOOB[, mygetPredictIds(glb_rsp_var, glbMdlSltId)$value],
                            glbObsOOB[, glb_rsp_var])$table))
}            

if (!is.null(glbFeatsCategory)) {
    glbLvlCategory <- merge(glbLvlCategory, 
                            myget_category_stats(obs_df = glbObsTrn, mdl_id = glbMdlFnlNslId, 
                                                 label = "trn"),
                            by = glbFeatsCategory, all = TRUE)
    row.names(glbLvlCategory) <- glbLvlCategory[, glbFeatsCategory]
    
    glbLvlCategory <- merge(glbLvlCategory, 
                            myget_category_stats(obs_df = glbObsNew, mdl_id = glbMdlFnlNslId,
                                                 label = "new"),
                            by = glbFeatsCategory, all = TRUE)
    row.names(glbLvlCategory) <- glbLvlCategory[, glbFeatsCategory]
...
if ((glb_rsp_var %in% names(glbObsNew)) &&
    ...
    if (glb_is_regression) {    
        pred_stats_df <- 
            mypredict_mdl(mdl = glb_models_lst[[glbMdlFnlNslId]], 
                          df = glbObsNew, 
                          rsp_var = glb_rsp_var, 
                          label = "new",
                model_summaryFunction = glb_sel_mdl$control$summaryFunction,
                          ...
                          ret_type = "stats")        
        print(sprintf("%s prediction stats for glbObsNew:", glbMdlFnlNslId))
        print(pred_stats_df)
    } else {   
        print(sprintf("%s new confusion matrix & accuracy: ", glbMdlFnlNslId))
        print(t(confusionMatrix(glbObsNew[, mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value],
...
print(orderBy(as.formula(paste0("~ -", gsub("#", ".", glbMdlSltId), ".imp")), 
              subset(tmpFeatsImp, imp.max > 10, select = -imp.max)))

dsp_myCategory_conf_mtrx <- function(myCategory) {
    print(sprintf("%s OOB::myCategory=%s confusion matrix & accuracy: ", 
                  glbMdlSltId, myCategory))
    print(t(confusionMatrix(
        glbObsOOB[glbObsOOB$myCategory == myCategory, 
                      paste0(mygetPredictIds(glb_rsp_var)$value, glbMdlSltId)], 
    ...
    print(sprintf("%s OOB::myCategory=%s FN errors: %d", glbMdlSltId, myCategory,
                  nrow(OOB_FNerr_df)))
    print(OOB_FNerr_df)
    ...
    OOB_FPerr_df <- glbObsOOB[(glbObsOOB$UniqueID %in% err_ids) & 
                        ...
                        c(
                            ".clusterid", 
                            "Popular", "Headline", "Snippet", "Abstract")]
    print(sprintf("%s OOB::myCategory=%s FP errors: %d", glbMdlSltId, myCategory,
                  nrow(OOB_FPerr_df)))
    print(OOB_FPerr_df)
}
...
if (glb_is_regression)
    print(myplot_histogram(glbObsNew, mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value))
if (glb_is_classification)
    print(table(glbObsNew[, mygetPredictIds(glb_rsp_var, glbMdlFnlNslId)$value]))
...
Consider adding a summary results chunk

```{r fix.this}
varImpDf <- caret::varImp(glb_models_lst[[glbMdlFnlNslId]])$importance
varImp <- gsub("#", ".", paste(glbMdlFnlNslId, "Imp", sep = "."), fixed = TRUE)
varImpDf[, varImp] <- varImpDf$Overall
#varImpDf <- dplyr::arrange_(varImpDf, .dots = desc(varImp))
varImpDf <- orderBy(as.formula(paste0("~ -", varImp)), varImpDf)
varImpDf[, "ftr"] <- row.names(varImpDf)
varImpDf <- subset(varImpDf, !grepl("^\\.", ftr))
print(myplot_hbar(varImpDf[1:min(10, nrow(varImpDf)),], "ftr", varImp))

# Summary plots by one feature
#ftr <- "Response.fctr"
#ftr <- "gCompany.fctr"
ftr <- "Datereceived.month.fctr"
pltDf <- glbObsTrn[, c(ftr, glb_rsp_var)] 
print(ggplot(data = pltDf, aes_string(x = ftr)) + 
        geom_bar(aes_string(y = "(..count..)/sum(..count..)", fill = glb_rsp_var)) + 
...
savFile <- paste0("data/", glbOut$pfx, "Results", ".RData")
print(sprintf("Saving file:%s; Objects:%s", savFile, paste0(savObjects, collapse = ",")))
save(list = savObjects, file = savFile)

#load(file = savFile, verbose = TRUE)
#load(file = savFile, envir = tmpEnv, verbose = TRUE)
